...
The typed N2 adapter now binds the exact HDF5 bytes, corrected result bytes, declared normalized subject hash, label-neutral typed subject projection, exact Fourier boundary, pressure, iota, flux, and current candidate/context membership. Focused tests pass 11/11 and 6/6; the adjacent typed 3-D input regression passes 56/56. This is exact representation, not a completed provider path. The source owns no interior radial extension or coordinate/metric program; `NFP=19` is outside the current interpreter range 2 through 8; the stored radial and vertical base-mode orientations fail the present analytic proof convention; and source `L=24` exceeds the fixed-boundary request cap 12. These remain ordered recoverable gaps. No signs were flipped, no radial powers were guessed, no resolution was reduced, and no provider or solver was executed.
