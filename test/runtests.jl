...
@testset "independent status dimensions and authority" begin
    ...
    @test_throws MethodError FinalWholeDeviceAuthorityV4(:whole_device)
    @test_throws MethodError AuthorityToken("subject", intermediate)
    @test_throws MethodError TerminalDecisionV4("subject", terminal_unsupported, intermediate)
    @test_throws ArgumentError final_closure("subject", FinalClosureInputV4((:a,), (), (true,), ("s",), (), (true,), ("missing",)), terminal_unsupported)
    closure = FinalClosureInputV4((:a,), (:a,), (true,), ("s",), ("s",), (true,), ())
    @test final_closure("subject", closure, terminal_unsupported).disposition == terminal_unsupported
end
...
@testset "conditional e-graph is derived and proof-bound" begin
    g = fixture_graph()
    cert = EquivalenceCertificateV1(canonical_hash(g), "a", "b", "rule", SideConditionProof("kappa is constant", SideConditionCheck("checker", true, "replay")))
    eg = derive_conditional_egraph(g, (cert,))
    @test eg.source_hash == canonical_hash(g)
    @test_throws ArgumentError SideConditionProof("", SideConditionCheck("checker", true, "replay"))
    @test_throws ArgumentError SideConditionCheck("checker", false, "replay")
end
